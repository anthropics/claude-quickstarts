...
import type { SandboxContainer } from "./container";
...
export interface Env {
  ...
  // "true" to let containers receive the environment key. This variant cannot
  // run a session any other way, so it refuses every work item until it is set.
  ALLOW_ENVIRONMENT_KEY_IN_SANDBOX?: string;
}
...
async function drainWork(env: Env, client: Anthropic): Promise<{ spawned: object[]; complete: boolean }> {
  ...
  for (let i = 0; i < MAX_DRAIN; i++) {
    ...
    // `ant beta:worker run` has no per-session credential, so this variant can
    // only run a session by handing the container the environment key, which
    // the agent's own bash can then read. The sibling providers refuse that
    // unless the operator opted in, and so does this one. It fails closed:
    // stop the item, so it is not left queued for a deploy that will never
    // take it.
    if ((env.ALLOW_ENVIRONMENT_KEY_IN_SANDBOX ?? "").toLowerCase() !== "true") {
      try {
        ...
        console.warn(`[webhook] could not stop refused work=${work.id}: ${errDetail(e)}`);
      }
      console.warn(
        `[webhook] REFUSED work=${work.id} session=${sessionId}: this variant puts the environment key in the container, and that is not enabled. Set ALLOW_ENVIRONMENT_KEY_IN_SANDBOX=true only if every session in this environment trusts every other. See the README, "One key, shared trust".`,
      );
      spawned.push({ session_id: sessionId, work_id: work.id, error: "environment_key_not_allowed" });
      continue;
    }
    try {
      ...
      if (!wasLive) {
        // The operator opted in above. Still say so on every start.
        console.warn(
          `[webhook] session=${sessionId}: the container receives the ENVIRONMENT KEY. Use this variant only when every session in this environment trusts every other.`,
        );
        await stub.dispatch({
          sessionId,
